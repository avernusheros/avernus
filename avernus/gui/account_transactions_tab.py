...
class AccountTransactionTab(gtk.VBox, page.Page):
    ...
    def _init_charts(self):
        ...
        categoryChart = charts.SimpleLineChart(over_time_controller,300)
        ...
        monthlyBtn.connect('toggled',lambda x: self.on_toggled(x,categoryChart, over_time_controller.set_monthly))
        ...
        valueChart = charts.SimpleLineChart(step_controller,300)
        self.charts.append((valueChart, step_controller))
        vbox = gtk.VBox()
        vbox.pack_start(valueChart, expand=True, fill=True)
        monthlyBtn = gtk.CheckButton(label=_('monthly'))
        monthlyBtn.connect('toggled',lambda x: self.on_toggled(x,valueChart, step_controller.set_monthly))
        hbox = gtk.HBox()
        hbox.pack_start(monthlyBtn, expand=False, fill=False)
        rollingAvgBtn = gtk.CheckButton(label=_('rolling average'))

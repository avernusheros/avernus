...
class AccountChartTab(gtk.VBox, page.Page):
    ...
    def show(self):
        ...
        if len(trans) > 0:
            ...
            self.table.attach(label, 0,2,y,y+1)
            chart_controller = chartController.TransactionValueOverTimeChartController(trans, (self.start_date, self.end_date))
            ...
            self.table.attach(label, 0,2,y,y+1)
            chart_controller = chartController.AccountBalanceOverTimeChartController(self.account, (self.start_date, self.end_date))
            chart = SimpleLineChart(chart_controller,width)
            self.updateable_charts.append((chart, chart_controller))
            self.table.attach(chart,0,2,y+1,y+2)
            y += 2
